...
#################### Arguments ####################
def parse_args():
    parser = argparse.ArgumentParser(description="Run MLP_SE.")
...
class MLP_SE():
    ...
    def __init__(self, num_users, num_items, n_neg_samples, reg_param, layers, epoch, t1_batch_size, t2_batch_size, learning_rate, keep_prob, optimizer_type, verbose, random_seed=2018):
        ...
        # self.t2_embedding_size = t2_embedding_size
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():  # , tf.device('/cpu:0'):
           ...
           # _________ Embedding Layer _____________
            ...
            # self.pool= tf.concat([self.u_emb, self.i_emb],1)
            # sself.pool = tf.reshape(self.pool, shape=[-1, self.layers[0]])
            self.u_emb = tf.reshape(self.u_emb, shape=[-1, self.layers[0]])
            self.i_emb = tf.reshape(self.i_emb, shape=[-1, self.layers[0]])
            self.pool = tf.multiply(self.u_emb, self.i_emb)
            # ________ Perception Layers __________
            for i in range(1, len(self.layers)):
                self.pool = tf.add(tf.matmul(self.pool, self.weights['layer_%d' % i]), self.weights['bias_%d' % i])  # None * layer[i] * 1
                self.pool = tf.nn.dropout(tf.nn.relu(self.pool), self.dropout_keep[i])  # dropout at each Deep layer
            # ________ Prediction Layer __________    
            self.predictions = tf.reduce_sum(tf.matmul(self.pool, self.weights['prediction']), 1, keep_dims=True)  # None * 1
            
            # Compute the loss.
            self.t1_loss = tf.nn.l2_loss(tf.subtract(self.ratings, self.predictions))
            
            # users=tf.unique(tf.reshape(self.users, shape=[-1]))[0]
            self._node_emb = tf.nn.embedding_lookup(self.weights['node_embeddings'], self.users) 
            # self._user_emb = tf.nn.embedding_lookup(self.weights['user_embeddings'], users )             
            self.t1_loss = self.t1_loss + self.reg_param[1] * tf.nn.l2_loss(tf.subtract(self.u_emb, self._node_emb))
            
            if self.reg_param[0] > 0:
                regularizer = tf.contrib.layers.l2_regularizer(self.reg_param[0])(self.i_emb)
                
                for i in range(1, len(self.layers)):
                     regularizer = regularizer + tf.contrib.layers.l2_regularizer(self.reg_param[0])(self.weights['layer_%d' % i]) + \
                                tf.contrib.layers.l2_regularizer(self.reg_param[0])(self.weights['bias_%d' % i]) 
            ...
            self.node_emb = tf.nn.embedding_lookup(self.weights['node_embeddings'], self.nodes)  # batch_size * embedding_size
...
if __name__ == '__main__':
    for reg1 in [0.01, 0.1, 1.0]:
        for reg_social in [0, 0.1, 1.0, 10.0]:
            dl = DataLoader('../datasets/douban/fold1')                    
            model = MLP_SE(num_users=dl.num_users, num_items=dl.num_items, n_neg_samples=10, reg_param=[reg1, reg_social],
                           layers=[64, 32, 16], keep_prob=[1, 1, 1], epoch=100, t1_batch_size=256, t2_batch_size=256, learning_rate=1e-4,
                           verbose=1, optimizer_type='AdamOptimizer')
            # model.pre_train_embedding(dl.Link_data, 100)
            model.train(dl.Train_data, dl.Test_data, dl.Link_data, '../log/mlpse.douban.log')
    for reg1 in [0.01, 0.1, 1.0]:        
        for reg_social in [0, 0.1, 1.0, 10.0]:
            ...
            model = MLP_SE(num_users=dl.num_users, num_items=dl.num_items, n_neg_samples=10, reg_param=[reg1, reg_social],
                           ...
                           verbose=1, optimizer_type='AdamOptimizer')
            # model.pre_train_embedding(dl.Link_data, 100)
            model.train(dl.Train_data, dl.Test_data, dl.Link_data, '../log/mlpse.ciaodvd.log')
    for reg1 in [0.01, 0.1, 1.0]:        
        for reg_social in [0, 0.1, 1.0, 10.0]:
            ...
            model.train(dl.Train_data, dl.Test_data, dl.Link_data, '../log/mlpse.epinion.log')        
