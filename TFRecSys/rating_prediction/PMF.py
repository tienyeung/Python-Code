...
class PMF:
    ...
    def __init__(self, n_users, n_items, embed_dim=50, learning_rate=0.001, reg_param=0.001, random_seed=2018):
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():  # , tf.device('/cpu:0'):
            ...
            # prediction    
            self.out = tf.reduce_sum(tf.multiply(self.u_emb, self.i_emb), 1, keepdims=True)
...
if __name__ == '__main__':
    ...
    for i in range(len(DATASET_NAMES)):
        ...
        log = open('../log/' + DATASET_NAMES[i] + '.PMF.log', 'a') 
        log.write("############################################### \n")
        log.write("batch_size=%d \n" % (BATCH_SIZE[i]))
        log.flush()      
        for embed_dim in [8, 16, 32, 64]:
            model = PMF(n_users=n_users, n_items=n_items, embed_dim=embed_dim, learning_rate=0.001)
