...
from sampler import RatingSampler, PairSampler, TrustSampler
from datatool import DataProcessor, CooccurrenceCounter
import pickle
...
class CRML:
    ...
    def __init__(self, nonlinear, n_users, n_items, embed_dim=50, margin=1.5, scaling_factor=3 / 4, cooccurrence_cap=100, alpha=0, beta=0, master_learning_rate=0.001,
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():  # , tf.device('/cpu:0'):
            ...
            #define a user_nonlinear_transform layer 
            self.user_nonlinear_transform = tf.layers.Dense(units=self.embed_dim, activation='tanh')
            self.item_nonlinear_transform = tf.layers.Dense(units=self.embed_dim, activation='tanh')
            ...
            if self.nonlinear is True:
                #Introduce non-linear transformation on embeddings of users/items
                users=self.user_nonlinear_transform.apply(users)
                pos_items=self.item_nonlinear_transform.apply(pos_items)
            ...
            #Introduce non-linear transformation on embeddings of users/items
            if self.nonlinear is True:
                neg_items=self.item_nonlinear_transform.apply(neg_items)
            ...
            if self.mode == 'USER' or self.mode == 'BOTH':
                ...
                self.cml_loss += self.alpha * tf.reduce_sum(tf.squared_difference(tf.nn.embedding_lookup(self.cml_weights['user_embeddings'], self.all_users_cml),
                                                                           tf.nn.embedding_lookup(self.glove_weights['glove_user_embeddings'], self.all_users_cml)))
            if self.mode == 'ITEM' or self.mode == 'BOTH':
                ...
                self.cml_loss += self.beta * tf.reduce_sum(tf.squared_difference(tf.nn.embedding_lookup(self.cml_weights['item_embeddings'], self.all_items_cml),
                                                                           tf.nn.embedding_lookup(self.glove_weights['glove_item_embeddings'], self.all_items_cml)))
            
            #self.cml_optimizer = tf.train.AdamOptimizer(self.master_learning_rate).minimize(self.cml_loss)
            self.cml_optimizer = tf.train.GradientDescentOptimizer(self.master_learning_rate).minimize(self.cml_loss)
            ...
            focal_user_embedding = tf.nn.embedding_lookup(self.glove_weights['glove_user_embeddings'], self.user_pairs[:, 0])
            context_user_embedding = tf.nn.embedding_lookup(self.glove_weights['glove_user_embeddings'], self.user_pairs[:, 1])
            focal_user_bias = tf.expand_dims(tf.nn.embedding_lookup(self.glove_weights['user_biases'], self.user_pairs[:, 0]), 1)
            context_user_bias = tf.expand_dims(tf.nn.embedding_lookup(self.glove_weights['user_biases'], self.user_pairs[:, 1]), 1)
            ...
            with tf.control_dependencies([self.glove_optimizer1]):
                 tf.assign(self.glove_weights['glove_user_embeddings'], tf.clip_by_norm(self.glove_weights['glove_user_embeddings'], self.clip_norm, axes=[1]))
            
            # placeholders
            self.item_pairs = tf.placeholder(tf.int32, shape=[None, 2])
            self.cooccurrence_count2 = tf.placeholder(tf.float32, shape=[None, 1])
            
            focal_item_emb = tf.nn.embedding_lookup(self.glove_weights['glove_item_embeddings'], self.item_pairs[:, 0])
            context_item_emb = tf.nn.embedding_lookup(self.glove_weights['glove_item_embeddings'], self.item_pairs[:, 1])
            focal_item_bias = tf.expand_dims(tf.nn.embedding_lookup(self.glove_weights['item_biases'], self.item_pairs[:, 0]), 1)
            context_item_bias = tf.expand_dims(tf.nn.embedding_lookup(self.glove_weights['item_biases'], self.item_pairs[:, 1]), 1)
            ...
            with tf.control_dependencies([self.glove_optimizer2]):
                 tf.assign(self.glove_weights['glove_item_embeddings'], tf.clip_by_norm(self.glove_weights['glove_item_embeddings'], self.clip_norm, axes=[1]))
            
            '''
            for evaluation part
            '''
            # (N_USER_IDS, 1, K)
            test_user_emb=tf.nn.embedding_lookup(self.cml_weights['user_embeddings'], self.score_user_ids)
            if self.nonlinear is True:
                test_user_emb=self.user_nonlinear_transform(test_user_emb)
            test_users = tf.expand_dims(test_user_emb, 1)
            # (1, N_ITEM, K)
            test_item_emb=self.cml_weights['item_embeddings']
            if self.nonlinear is True:
                test_item_emb=self.item_nonlinear_transform(test_item_emb)
            test_items = tf.expand_dims(test_item_emb, 0)
    ...
    def train(self, log, cml_sampler, user_pair_sampler, item_pair_sampler, train, test, iters, top_k):
        ...
        for i in range(iters):
            ...
            if self.mode == 'ITEM':
                glove_loss = glove_loss2
            if self.mode == 'USER':
                glove_loss = glove_loss1
            if self.mode == 'ITEM' or self.mode == 'USER':
                print("\nTraining loss: %f (%f, %f) in %.2fs" % (numpy.sum(losses), glove_loss, cml_loss, t2 - t1))
                log.write("Training loss: %f (%f, %f) in %.2fs \n" % (numpy.sum(losses), glove_loss, cml_loss, t2 - t1))
            if self.mode == 'BOTH':
                print("\nTraining loss: %f (%f, %f, %f) in %.2fs" % (numpy.sum(losses), glove_loss1, glove_loss2, cml_loss, t2 - t1))
                log.write("Training loss: %f (%f, %f, %f) in %.2fs \n" % (numpy.sum(losses), glove_loss1, glove_loss2, cml_loss, t2 - t1))
            if self.mode == 'NONE':
...
if __name__ == '__main__':
    BATCH_SIZE = [1024, 1024, 2096]
    TOP_K = [1, 5, 10]
    MAX_ITERS = [100, 100, 30]
    N_NEGATIVE = [20, 20, 20]
    DATASET_NAMES = ['lastfm', 'andriod_app','ml-100k', 'filmtrust', 'ml-100k']
    ...
    for i in range(len(DATASET_NAMES)):
        ...
        #pickle.dump(dp.est_cooccurrence_of_users(train),open('datasets/' + DATASET_NAMES[i] + '/user_cooc.pkl',"wb"))
        ...
        for embed_dim in [100]:
            for margin in [2.0]:
                ...
                #===============================================================
                model = CRML(nonlinear=True, n_users=n_users, n_items=n_items, embed_dim=embed_dim, margin=margin, clip_norm=1, alpha=0, beta=0,  master_learning_rate=0.01, mode=MODE[3])
                model.train(log, cml_sampler, user_pair_sampler, item_pair_sampler, train, test, MAX_ITERS[i], TOP_K)
                # 
                #===============================================================
                model = CRML(nonlinear=True,n_users=n_users, n_items=n_items, embed_dim=embed_dim, margin=margin, clip_norm=1, alpha=1.0, beta=0, master_learning_rate=0.0001, mode=MODE[0])
                model.train(log, cml_sampler, user_pair_sampler, item_pair_sampler, train, test, MAX_ITERS[i], TOP_K)
                        
                model = CRML(nonlinear=True,n_users=n_users, n_items=n_items, embed_dim=embed_dim, margin=margin, clip_norm=1, alpha=0, beta=1.0, master_learning_rate=0.0001, mode=MODE[1])
                model.train(log, cml_sampler, user_pair_sampler, item_pair_sampler, train, test, MAX_ITERS[i], TOP_K)
                
                model = CRML(nonlinear=True,n_users=n_users, n_items=n_items, embed_dim=embed_dim, margin=margin, clip_norm=1, alpha=1.0, beta=1.0, master_learning_rate=0.0001, mode=MODE[2])
                model.train(log, cml_sampler, user_pair_sampler, item_pair_sampler, train, test, MAX_ITERS[i], TOP_K)
                        
                for alpha in [0, 0.01, 0.1, 1.0]:
                    for beta in [0, 0.01, 0.1, 1.0]:
                        
                        model = CRML(nonlinear=True,n_users=n_users, n_items=n_items, embed_dim=embed_dim, margin=margin, clip_norm=1, alpha=alpha, beta=beta, master_learning_rate=0.001, mode=MODE[2])
                        model.train(log, cml_sampler, user_pair_sampler, item_pair_sampler, train, test, MAX_ITERS[i], TOP_K)
                #===============================================================
