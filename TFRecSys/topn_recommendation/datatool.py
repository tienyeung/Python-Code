...
class CooccurrenceCounter:
    ...
    def __init__(self, user_item_matrix, n_workers):
        self.user_item_matrix = user_item_matrix
        self.result_queue = Queue()
        self.n_workers = n_workers
        
    def est_cooccurrence_of_users(self, min_count=1):  
        processors = []
        id_list = [i for i in range(self.user_item_matrix.shape[0])]
        np.random.shuffle(id_list)
        batch_size = int(len(id_list) / self.n_workers + 0.5)
        for n in range(self.n_workers):
            list = []
            if n < self.n_workers - 1:
                id_sublist = id_list[n * batch_size:(n + 1) * batch_size]
            if n == self.n_workers - 1:
                id_sublist = id_list[n * batch_size:]
            processors.append(Process(target=self.est_cooccurrence, args=(self.user_item_matrix, id_sublist, self.result_queue, min_count)))
            processors[-1].start()
            
        cooccurrence_list = []
        for p in processors:  # type: Process
            cooccurrence_list.extend(self.result_queue.get())
            p.terminate()
            p.join()  
        print('#co-occurrence entries of users:%d' % (len(cooccurrence_list)))
        return cooccurrence_list
    
    def est_cooccurrence_of_items(self, min_count=1):  
        processors = []
        id_list = [i for i in range(self.user_item_matrix.shape[1])]
        np.random.shuffle(id_list)
        batch_size = int(len(id_list) / self.n_workers + 0.5)
        for n in range(self.n_workers):
            list = []
            ...
            if n == self.n_workers - 1:
                id_sublist = id_list[n * batch_size:]
            processors.append(Process(target=self.est_cooccurrence, args=(self.user_item_matrix.transpose(), id_sublist, self.result_queue, min_count)))
            processors[-1].start()
        ...
        for p in processors:  # type: Process
            ...
            p.terminate()
            p.join()  
        print('#co-occurrence entries of items:%d' % (len(cooccurrence_list)))
        return cooccurrence_list
            
    def est_cooccurrence(self, feedback_matrix, id_sublist, result_queue, min_count=1):
        num_objects = feedback_matrix.shape[0]
        cooccurrence_list = list()
        row_to_column_set = {r: set(row) for r, row in enumerate(lil_matrix(feedback_matrix).rows)}
        for i in id_sublist:
            for j in range(i + 1, num_objects):
                co_count = len(row_to_column_set[i] & row_to_column_set[j])
                if co_count >= min_count: 
                    cooccurrence_list.append([i, j, co_count])
        result_queue.put(cooccurrence_list)                
