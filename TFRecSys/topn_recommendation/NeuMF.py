...
#            self.labels = tf.placeholder(tf.float32, [None])
            ...
            with tf.name_scope("mlp"):  
                ...
                for idx in range(1, len(self.layers)):
                    self.mlp.append(tf.layers.Dense(units=self.layers[idx], kernel_regularizer= tf.contrib.layers.l2_regularizer(self.reg_layers[idx]), activation='relu', name = 'layer%d' %idx))
            ...
            +tf.contrib.layers.l2_regularizer(0.001)(self.weights["mlp_user_embeddings"])\
            +tf.contrib.layers.l2_regularizer(0.001)(self.weights["mlp_item_embeddings"])\
            +tf.contrib.layers.l2_regularizer(0.001)(self.weights["mf_user_embeddings"])\
            +tf.contrib.layers.l2_regularizer(0.001)(self.weights["mf_item_embeddings"])
